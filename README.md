...
graph TD
    ...
    # 4) Arquitectura del Pipeline y Justificación de Componentes

Para dar respuesta a los requerimientos del negocio y maximizar la velocidad de desarrollo, se optó por un esquema tecnológico local y portable (Plataforma de comodidad del equipo). A continuación, se detalla la justificación técnica de cada componente seleccionado para las capas del pipeline:

---

### 📥 b) Ingesta de Datos (Data Ingestion)
* **Servicio/Herramienta Seleccionada:** Python 3 (`requests` + `argparse`).
* **Justificación Breve:** La librería nativa `requests` permite gestionar de manera síncrona, ligera y eficiente las peticiones HTTP/REST hacia los endpoints del Banco Mundial, CEPAL y Chile Abierto. Adicionalmente, la incorporación de `argparse` dota al pipeline de una interfaz de línea de comandos (CLI) que permite parametrizar las consultas (por país o rango de años) dinámicamente desde la terminal, facilitando ejecuciones tanto interactivas como automatizadas (vía scripts de Bash o tareas Cron).

---

### 🗄️ c) Almacenamiento (Data Storage)
* **Servicio/Herramienta Seleccionada:** Archivos Parquet y Base de Datos Relacional **SQLite** (`.db`).
* **Justificación Breve:** * El formato **Parquet** se justifica en la capa cruda (*Raw*) debido a su almacenamiento columnar y compresión nativa (Snappy), lo que optimiza el espacio en disco y acelera las transferencias de datos analíticos de alta densidad.
  * **SQLite** se justifica para el modelo de datos unificado debido a que almacena toda la base de datos relacional en un único archivo plano y portable. Esto elimina los costos de administración, aprovisionamiento de red y configuraciones de servidores tradicionales (como PostgreSQL o MySQL), garantizando que cualquier evaluador pueda clonar el repositorio y contar con el motor de datos operativo de inmediato.

---

### ⚙️ d) Procesamiento (Data Processing / ETL)
* **Servicio/Herramienta Seleccionada:** Python Ecosystem (**Pandas / Polars**).
* **Justificación Breve:** Pandas es el estándar de la industria para la manipulación tabular de datos de mediana escala en memoria RAM. Permite ejecutar en microsegundos las uniones externas combinadas (*Outer Merges*) para alinear las series cronológicas compartiendo la llave temporal `Anio`. Su uso está justificado para centralizar las reglas de negocio (limpieza de nulos, tipado de datos y filtrado estricto al rango 1960-2026), evitando sobrecargar la capa de visualización con cálculos pesados.

---

### 📊 e) Visualización (Data Visualization / Consumption)
* **Servicio/Herramienta Seleccionada:** **Power BI Desktop** o **Streamlit** (App Python).
* **Justificación Breve:** * Si se opta por la ruta *Low-Code*, **Power BI Desktop** ofrece controladores de conexión local nativos para SQLite, lo que permite prototipar visualizaciones analíticas interactivas en pocas horas a costo cero.
  * Si se opta por la ruta *Full-Code*, **Streamlit** permite construir interfaces web dinámicas directamente en Python. Se justifica por su velocidad de desarrollo y su integración nativa con *Streamlit Community Cloud*, plataforma que permite desplegar el dashboard funcional en la web de manera pública, gratuita y en un par de clics, cumpliendo estrictamente con el requisito de despliegue del trabajo.

---

## 🗺️ Diagrama de Arquitectura de Alto Nivel

El siguiente diagrama representa la integración conceptual y técnica de los servicios elegidos, mostrando el flujo síncrono del dato a través de las cuatro capas obligatorias del proyecto:
...
graph TD
    ...
    subgraph Procesamiento [d. Capa de Procesamiento / ETL]
        PD_ETL[Motor Pandas / Polars <br> Limpieza, Outer Merge y KPIs]
    end
